/* global dbStorage, AudioEngine, extraToolsSource, permanentTools, M, grid */
/* global brushSize:writable, eraserSize:writable, fireIntensity:writable, fireChaos:writable, simulationSpeed:writable, musicEnabled:writable, sfxEnabled:writable, activeExtraTools:writable, allCurrentTools:writable, currentMaterial:writable, currentEditingWorldId:writable, currentEditingWorldName:writable */
/* global refreshUserWorlds, resetWorld, renderBigInventory, updateEditStatusUI, loadUserWorld, deleteUserWorld, exitEditMode, setMaterial, selectExtraTool */
...
async function saveAllSettings() {
    const settings = {
        ...
        // Music/SFX always true, no need to save or we save true to be safe
        musicEnabled: true,
        sfxEnabled: true,
...
function openSaveWorldDialog() {
    document.getElementById('saveDialog').classList.remove('hidden');
    const input = document.getElementById('worldNameInput');
    // Pre-fill with current name if available, to make overwriting easier
    if (currentEditingWorldName) {
        input.value = currentEditingWorldName;
    }
    input.focus();
}
...
async function saveCurrentWorld() {
    const nameInput = document.getElementById('worldNameInput').value.trim();
    const name = nameInput || 'Unbenannt';

    // Check if world with this name exists to overwrite it
    const allWorlds = await dbStorage.getAllWorlds();
    const existing = allWorlds.find(w => w.name.toLowerCase() === name.toLowerCase());

    const id = existing ? existing.id : 'user-' + Date.now();

    // Use ChunkEngine export
    const worldData = ChunkEngine.exportWorld();
    await dbStorage.saveWorld(id, name, worldData);

    // Update current session info so we know what we are working on
    currentEditingWorldId = id;
    currentEditingWorldName = name;
    updateEditStatusUI(); // Show name of currently loaded/saved world
...
function updateEditStatusUI() {
    ...
    // Show name if we have one, but this doesn't imply autosave anymore
    if (currentEditingWorldName) {
        status.classList.remove('hidden');
        nameEl.innerText = currentEditingWorldName;
        // Update the label text to "WELT:"
        const label = status.querySelector('span');
        if (label && label.childNodes[0].nodeType === 3) {
            label.childNodes[0].textContent = "WELT: ";
        }
    } else {
...
// Deprecated: Autosave removed as per user request
...
async function refreshUserWorlds() {
    ...
    worlds.sort((a, b) => b.timestamp - a.timestamp).forEach(w => {
        ...
        row.className = "flex justify-between items-center group bg-white/5 hover:bg-white/10 p-2 sm:p-1 rounded transition-all mb-1 min-h-[30px]";

        row.innerHTML = `
            <button onclick="loadUserWorld('${w.id}')" class="flex-1 text-left truncate text-[10px] hover:text-yellow-400 min-w-0 pr-2 overflow-hidden">
                📁 ${w.name.toUpperCase()}
            </button>
...
async function loadUserWorld(id) {
    ...
    if (w) {
        resetWorld();
        // Use ChunkEngine import
        ChunkEngine.importWorld(w.data);
        toggleWorlds(); // Close panel

        // We set the ID/Name so the user knows what they loaded, 
        // and so "Save" defaults to this name.
        currentEditingWorldId = w.id;
        currentEditingWorldName = w.name;
        updateEditStatusUI();
    }
}
