// --- PHYSICS UPDATE ---
// Optimizations based on optimierung.md:
// 1. Fast RNG (random() instead of Math.random())
// 2. DOM Caching (avoid getElementById in loop)
// 3. Interval Heatmap Update (only every 3rd frame)
...
function updatePhysics() {
    ...
    // 0. MELTDOWN MANAGEMENT (Optimized DOM Access)
    ...
    // 1. HEATMAP & FEUER LOGIK (Throttled: Run only every 3rd frame)
    ...
    for (let y = 0; y < height; y++) {
        ...
        for (let i = 0; i < width; i++) {
            ...
            if (type === M.FIREWORK) {
                if (fireworkVel[idx] === 0) fireworkVel[idx] = 240 + Math.floor(random() * 30);
                        ...
                        for (let ex = -6; ex <= 6; ex++) {
                            if (ex * ex + ey * ey <= 36 && random() < 0.5) {
                                ...
                                if (sx >= 0 && sx < width && sy >= 0 && sy < height) {
                                    ...
                                    if (grid[si] === M.AIR) { grid[si] = M.SPARK; sparkColors[si] = color; heatMap[si] = 160 + random() * 95; }
                                }
                            }
                        }
                    }
                ...
                if (y > 0 && grid[idx - width] === M.AIR) { grid[idx - width] = M.STEAM; grid[idx] = M.AIR; } else if (random() < 0.05) grid[idx] = M.AIR;
                    ...
                    for (let dy = -2; dy <= 2; dy++) {
                        for (let dx = -2; dx <= 2; dx++) {
                            ...
                            if (bi >= 0 && bi < grid.length) {
                                ...
                                else if ((grid[bi] === M.WOOD || grid[bi] === M.TERMITE) && random() < 0.3) { grid[bi] = M.FIRE; heatMap[bi] = 255; }
                            }
                        }
                    }
                } else {
                ...
                const strike = (sIdx, en) => {
                    ...
                    while (en > 0 && cur < width * height) {
                        ...
                        const t = grid[b];
                        if (t !== M.AIR && t !== M.METHANE && t !== M.STEAM && t !== M.SPARK && t !== M.LIGHTNING) {
                            if (t === M.WATER) grid[b] = M.STEAM; else { grid[b] = M.STONE; grid[cur] = M.EMBER; }
                            heatMap[b] = 255; break;
                        } else if (t === M.METHANE) { grid[b] = M.FIRE; heatMap[b] = 255; }
                        let d = width; const r = random(); if (r < 0.2) d -= 1; else if (r < 0.4) d += 1;
                        if (en > 10 && random() < 0.05) strike(cur, en - 5);
                        cur += d; en--;
                    }
                };
            ...
            if (type === M.FIRE) {
                ...
                for (let offset of spreads) {
                    ...
                    if (nIdx >= 0 && nIdx < width * height) {
                        ...
                        } else if (nType === M.WATER) { grid[nIdx] = M.STEAM; if (random() < 0.5) grid[idx] = M.AIR; }
                    }
                }
                if (random() < 0.4) {
                    const wIdx = idx + (random() > 0.5 ? 1 : -1);
                    if (grid[wIdx] === M.AIR && Math.floor(wIdx / width) === y) { grid[wIdx] = M.FIRE; grid[idx] = M.AIR; }
                }
                if (random() < 0.25) grid[idx] = (random() > 0.3) ? M.STEAM : M.AIR;
            }
            else if (type === M.SAND) {
                const below = idx + width;
                if (y < height - 1) {
                ...
                if (y < height - 1) {
                    if (grid[below] === M.AIR) { grid[below] = M.WATER; grid[idx] = M.AIR; }
                        ...
                        let fD = liquidDir[idx] || (random() < 0.5 ? 1 : -1);
                        const next = idx + fD;
                ...
                for (let dy = -1; dy <= 1; dy++) {
                    for (let dx = -1; dx <= 1; dx++) {
                        ...
                        if (ni >= 0 && ni < grid.length && [M.WOOD, M.LEAVES, M.VINE, M.TERMITE].includes(grid[ni]) && random() < 0.02) { grid[ni] = M.FIRE; heatMap[ni] = 255; }
                ...
                if (!meltdownActive) {
                    if (heatMap[idx] >= 250 || random() < (0.0002 * (simulationSpeed / 0.4))) {
                        meltdownActive = true; meltdownTimer = 600; // 10s countdown
                    }
                }
                if (meltdownActive) {
                    const pulseDist = Math.sin(globalTime * 6) * 25 + 25;
                    const rIdx = idx + Math.floor((random() - 0.5) * pulseDist) + Math.floor((random() - 0.5) * pulseDist) * width;
                    if (rIdx >= 0 && rIdx < grid.length && grid[rIdx] !== M.AIR && grid[rIdx] !== M.URANIUM && random() < 0.2) {
                    ...
                    if (meltdownTimer <= 0) {
                        ...
                        for (let dy = -r; dy <= r; dy++) {
                            for (let dx = -r; dx <= r; dx++) {
                                ...
                                if (tx >= 0 && tx < width && ty >= 0 && ty < height) {
                                    const ti = ty * width + tx;
                                    if (d2 < rSq * 0.5) grid[ti] = M.AIR; else if (random() < 0.8) grid[ti] = M.LAVA;
                                    heatMap[ti] = 255;
                                }
                            }
                        }
                ...
                if (below < width * height) {
                    ...
                    if (bt === M.AIR) { grid[below] = M.ACID; grid[idx] = M.AIR; }
                        ...
                        if (bt === M.CONCRETE) { if (random() < 0.02) { grid[below] = M.AIR; grid[idx] = M.AIR; } }
                        else { grid[below] = M.AIR; if (random() < 0.15) grid[idx] = M.AIR; }
                    }
                } else {
                    let fD = liquidDir[idx] || (random() < 0.5 ? 1 : -1);
                ...
                if (below < width * height && grid[below] === M.AIR && (!touchW || random() < 0.1)) { grid[below] = M.TERMITE; grid[idx] = M.AIR; continue; }
                if (random() < 0.6) {
                    const r = random();
                    let m = (r < 0.45) ? width : (r < 0.75 ? (random() < 0.5 ? 1 : -1) : (touchW ? -width : (random() < 0.5 ? 1 : -1)));
                    const ds = idx + m;
                    if (ds >= 0 && ds < grid.length && (Math.abs(m) !== 1 || Math.floor(ds / width) === y)) {
                        const dt = grid[ds];
                        if ([M.WOOD, M.LEAVES, M.VINE].includes(dt)) { grid[ds] = M.TERMITE; grid[idx] = M.AIR; if (random() < 0.1) grid[idx] = M.METHANE; }
                        else if (dt === M.AIR && random() < 0.15) { grid[ds] = M.TERMITE; grid[idx] = M.AIR; }
                    }
                }
            }
            else if (type === M.VINE) {
                if (heatMap[idx] > 80) { grid[idx] = M.FIRE; heatMap[idx] = 200; continue; }
                if (random() < 0.015) { // Slightly increased spread
                ...
                if (random() < 0.05) { const n = idx + (random() < 0.5 ? 1 : -1); if (n >= 0 && n < grid.length) heatMap[n] = Math.max(0, heatMap[n] - 10); }
            }
            else if (type === M.COAL) {
                if (heatMap[idx] > 200) {
                    heatMap[idx] = 255;
                    if (random() < 0.05) {
                        const sides = [idx - 1, idx + 1, idx - width, idx + width];
                        const s = sides[Math.floor(random() * sides.length)];
                        if (s >= 0 && s < width * height && grid[s] === M.AIR) grid[s] = M.FIRE;
                    }
                }
            }
            else if (type === M.EMBER) {
                // Ember can now also spread heat/fire slowly
                if (random() < 0.02) {
                    const nb = [idx - 1, idx + 1, idx - width, idx + width];
                    const randomNb = nb[Math.floor(random() * nb.length)];
                    if (randomNb >= 0 && randomNb < width * height) {
                        if (grid[randomNb] === M.AIR && random() < 0.1) grid[randomNb] = M.FIRE;
                        else if ([M.WOOD, M.LEAVES, M.VINE, M.COAL].includes(grid[randomNb])) heatMap[randomNb] = Math.min(255, heatMap[randomNb] + 50);
                    }
                }
                if (random() < 0.1) {
                    const nb = [idx - 1, idx + 1, idx - width, idx + width];
                    for (let n of nb) if (n >= 0 && n < width * height && grid[n] === M.FIRE) { grid[idx] = M.COAL; heatMap[idx] = 255; break; }
                }
            }
            else if (type === M.SPARK) {
                heatMap[idx] -= 6; if (heatMap[idx] <= 0 || random() < 0.05) { grid[idx] = M.AIR; sparkColors[idx] = 0; }
                else { const b = idx + width; if (b < grid.length && grid[b] === M.AIR && random() < 0.3) { grid[b] = M.SPARK; sparkColors[b] = sparkColors[idx]; heatMap[b] = heatMap[idx]; grid[idx] = M.AIR; } }
            }
            else if (type === M.WOOD || type === M.LEAVES) {
                if (heatMap[idx] > 120 && random() < 0.02) { grid[idx] = M.FIRE; heatMap[idx] = 255; }
            }
        }
    }

    if (currentlyHasUranium && !meltdownActive) {
        if (random() < (0.0005 * (simulationSpeed / 0.4))) { meltdownActive = true; meltdownTimer = 600; }
    }
